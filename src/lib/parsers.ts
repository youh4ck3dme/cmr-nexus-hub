...
function parseLeadBlock(block: string, reportId: string): Lead | null {
  ...
  const label =
    (extractMatch(block, /(?:Score\s*Label|Label)\s*[:-]\s*(KEEP|BORDERLINE|REJECTED)/i) as
      ScoreLabel | undefined) ?? pickScoreLabel(scoreTotal, scoreMax);
  ...
  if (revenue && /odhad|estimate|~|approx/i.test(revenue))
    warnings.push("ESTIMATED_REVENUE_IMPACT");
...
export function parseLeadReport(raw: string): ParsedReportPreview {
  ...
  const leads = blocks.map((b) => parseLeadBlock(b, reportId)).filter((l): l is Lead => l !== null);
    ...
    keepCount > 0
      ? Math.round(
          leads
            ...
            .reduce((s, l) => s + (l.score_total / (l.score_max || 100)) * 100, 0) / keepCount,
        )
      : undefined;

  const warnings: string[] = [];
  if (leads.length === 0)
    warnings.push("Nepodarilo sa rozpoznať žiadne leady – skontroluj formát.");
...
export function parseMessage(raw: string): ParsedMessage {
  ...
  const website = raw.match(
    /https?:\/\/[^\s]+|(?:www\.)?[a-z0-9-]+\.(?:com|sk|co\.uk|io|net|org|dev|app)(?:\/\S*)?/i,
  )?.[0];
  const budget = raw.match(/[€$£]\s?\d[\d\s,.]*|(?:\d[\d\s,.]*)\s?(?:€|EUR|GBP|USD)/i)?.[0];
  const service = raw.match(
    /(?:booking|redesign|web|CRM|dashboard|app|PWA|SEO|automation|integr[aá]cia|integrace|integration)[a-z\s]{0,30}/i,
  )?.[0];
  const contact = raw.match(
    /(?:from|od|volá|hi,?\s+I['’]?m|som)\s+([A-ZÁ-Ž][\wá-ž]+(?:\s[A-ZÁ-Ž][\wá-ž]+)?)/i,
  )?.[1];
  const companyGuess = raw.match(
    /(?:for|pre|from|z)\s+([A-ZÁ-Ž][\w&.-]+(?:\s+[A-ZÁ-Ž][\w&.-]+){0,3})/,
  )?.[1];
